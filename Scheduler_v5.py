...
def add_agent(agents_list):
    try:
        name = str(input("Hello, Please start by giving me your name: "))
        if len(name) < 2:
            print("Invalid input for Name, must be at least 2 characters long")
            return
        else:
            print(f"Hello {name}, thank you for joining us!")
            agent_id = str(input("Please enter your Agent ID: "))
            type_fte = str(input("Please enter your FTE type PT/FT: "))
            FTE = float(input("Please enter your FTE: "))
            days_worked = list(input("Please enter the days you worked (monday being 0 and sunday 6 - no space or comma): "))
            hours_worked = tuple(map(int, input("Please enter the range of hours you work each day (e.g., 8,18): ").split(",")))
            agents_list.append({
            "Agent ID": agent_id,
            "name": name,
            "type": type_fte,
...
def menu():
    ...
    print("1. Run Full Period Solver")
    print("2. Add Agent")
    print("3. Count Agents")
    print("4. Exit")

def count_agents(agents_db):
    print(f"Total agents: {len(agents_db)}")
    ...
    while True:
        ...
        try:
            ...
            if choice_user ==1:
                ...
                count_agents(agents_db)
            elif choice_user == 4:
                print("OK thanks, see you soon!")
                break
            else:
